...
#sound
@onready var snake_hiss: AudioStreamPlayer2D = $snake_hiss
@onready var snake_bite: AudioStreamPlayer2D = $snake_bite
...
func update_animation():
	match state:
		...
		State.WALK:
			anim.play("walk")
			if not snake_bite.playing and randf() < 0.02:
				snake_hiss.play()
...
func die():
	...
	if drop_item_scene and randf() <= drop_chance:
		var buff = drop_item_scene.instantiate()
